...
\begin{itemize}
\item \textbf{model}. Contiene toda la lógica del programa. Además de todos los paquetes que aparecen a continuación, contiene las clases que conforman el patrón \textit{Observador} (Observer.java y Observable.java) y la clase Model.java que gestiona el grueso de la lógica.
	\begin{itemize}
	\item \textbf{mutantoperator}. Recoge la clase abstracta MutantOperator.java que gestiona un operador de mutación.
		\begin{itemize}
		...
		\end{itemize}		 
	\item \textbf{mutant}. Contiene la clase Mutant.java que gestiona un mutante tras su creación: ruta al archivo mutante, ruta al archivo original y línea de código que sufrió la mutación.
	\item \textbf{test}. Contiene la clase abstracta Test.Java que gestiona las características del tipo de prueba a realizar, por ejemplo si es o no determinista. También contiene sus implementaciones.
	...
	\item \textbf{files}. Contiene la clase TestFile.java que gestiona los archivos finales creados por las clases del paquete \textit{run}.
	\item \textbf{testresult}. Contiene la clases abstracta TestResult.java que gestiona los resultados de la ejecución de los test y también sus implementaciones, una por cada implementación de Test.java.
	\end{itemize}
\item \textbf{view}. Encargado de toda la vista del sistema.
	...
	\end{itemize}
\item \textbf{control}. Contiene la clase Control.java que actúa como controlador del patrón MVC.
\item \textbf{exception}. Recoge algunas excepciones del programa.
\item \textbf{main}. Recoge la clase Main.java que contiene el método que inicia la ejecución del programa.
\end{itemize}
...
Los componentes de esta vista se encuentran en el paquete view.mutantgeneratorview, aunque además hace uso de clases del paquete view.tools. En concreto, JTableCheck.java, que gestiona una tabla de  objectos de una determinada instancia junto a casillas verificadoras asociadas a booleanos y LogArea.java que imprime información para el usuario.

% Hablar de archivos (ejecución 1 a 1, etc)
% Hablar de operadores
% Hablar de los paquetes de la lógica
% Tal vez contar lo de unico archivo como problema.

\begin{figure}
\begin{center}
\includegraphics[scale=0.5]{images/vista1}
\end{center}
...
\begin{figure}[!h]
\begin{center}
\includegraphics[scale=0.5]{images/vista2}
\end{center}
...
\begin{figure}[!h]
\begin{center}
\includegraphics[scale=0.5]{images/vista3}
\end{center}
\caption{Vista del subsistema pruebas de mutación y creación de test.}
\label{fig:vista3}
\end{figure}
\subsubsection{Subsistemas: Visualizador de reultados}

\begin{figure}[!h]
\begin{center}
\includegraphics[scale=0.5]{images/vista4}
\end{center}
