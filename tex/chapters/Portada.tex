%----------------------------------
%Inicio portada
	\begin{titlepage}
\begin{center}
\vspace*{+0.15in}
\textbf{\textsc{\begin{Huge}Computación cuántica:\\Pruebas de mutación\end{Huge}}}

\vspace*{+0.15in}
\begin{figure}[htb]
\begin{center}
\includegraphics[width=8cm]{chapters/complu}\\\ \\\ \\
\textsc{\textbf{\begin{LARGE}Universidad Complutense de Madrid\end{LARGE}}}\\\ \\\ \\\ \\
\textsc{\begin{Large}Facultad de Informática\end{Large}}\\\ \\
\textsc{\begin{large}Doble Grado en Matemáticas e Ingeniería Informática\end{large}}
\end{center}
\end{figure}

\vspace*{0.15in}
\textsc{\textbf{\begin{large} Tutor: Manuel Núñez García\end{large}}\\}
\vspace*{0.15in}

\textsc{\textbf{\begin{large} Luis Aguirre Galindo \& Javier Pellejero Ortega\end{large}}\\}
Curso 2019-2020
\end{center}
\end{titlepage}
% Fin portada
%-------------------------------------------------------

\blankpage

\pagenumbering{Roman} % para comenzar la numeracion de paginas en numeros romanos
	\chapter*{}
\begin{flushright}
\textit{Un matemático que no es en algún sentido un\\poeta no será nunca un matemático completo}.\\\ \\ Karl Weierstra\ss
\end{flushright}

	\chapter*{Prefacio}
	
	
	
\mainmatter % Empieza a numerar en números arábigos 

	\tableofcontents